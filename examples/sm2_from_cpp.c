...
int PriKey2PubKey(char *in_priKey, char **out_pubKey)
{
	BIO* keybio = NULL;
...
int Sign(char *in_buf, int in_buflen, char *out_sig, int *len_sig, char *priKey)
{
	...
	if (1 != ECDSA_sign(0, (const unsigned char*)in_buf, in_buflen, szSign, (unsigned int*)len_sig, eckey)) {
...
int Verify(char *in_buf, const int buflen, char *sig, const int siglen,
